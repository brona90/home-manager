{
  ...
  in {
    ...
    # Every guard lives in checks/, one file per concern, so that two branches
    # touching two different guards do not conflict. checks/default.nix
    # documents the interface; everything the guards need is passed explicitly
    # below, because nothing under checks/ can see this `let`.
    checks = forAllSystems (
      system:
        import ./checks {
          inherit (nixpkgs) lib;
          inherit system homeConfigs userForSystem devShellFor;
          pkgs = pkgsFor system;
        }
    );
  };
