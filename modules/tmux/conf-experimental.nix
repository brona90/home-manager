...
#            applies the chosen theme at conf load and on prefix-T cycle.
{
  ...
  # Set terminal-emulator window title (matches gpakosz default).
  set -g set-titles on
  ...
  # --- Keybindings -------------------------------------------------------
  # Edit the tmux config source (gpakosz prefix-e parity). Opens the nix
  # module in $EDITOR; user runs `home-manager switch` then prefix-r to
  # apply. (Source-and-reload like gpakosz isn't possible here -- the live
  # conf is /nix/store read-only, regenerated by home-manager.)
  bind e new-window -n "tmux-config" "$EDITOR ~/.config/home-manager/modules/tmux/"
  ...
  # Buffers
  # prefix b lists, prefix p pastes top, prefix B chooses (gpakosz had
  # choose-buffer on prefix P; remapped here because P now opens the
  # project picker).
  bind b list-buffers
